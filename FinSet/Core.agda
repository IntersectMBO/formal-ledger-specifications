...
open import Function.Equivalence using (_⇔_; ⇔-setoid; equivalence)
...
set-monad-ht : {{_ : DecEq X}} {{_ : DecEq Y}} {b1 b2 : Bool}
 → (e : Y)(xs : FiniteSubSet X _≟_ b1)(f : X → FiniteSubSet Y _≟_ b2) → (x : X) (b : Bool)
 → x ∈l listOf xs
 → e ∈l listOf (f x)
 → e ∈l (listOf (bind xs f b))
...
set-monad-th : {{_ : DecEq X}} {{_ : DecEq Y}} {b1 b2 : Bool}
