...
source_path: src/Test/Examples/AccountSim/OffChain/Deposit.lagda.md
---
```agda
{-# OPTIONS --safe #-}
...
scriptDTxOut : Datum → TxOut → (w : ℕ) → (v : Value) → TxOut
...
makeDepositTxOut : Datum → (scriptIx : ℕ) → TxOut → (w : ℕ) → (v : Value) → List (ℕ × TxOut)
