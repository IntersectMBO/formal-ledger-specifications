...
source_path: src/Test/Examples/AccountSim/OffChain/Withdraw.lagda.md
---
```agda
{-# OPTIONS --safe #-}
...
scriptTxOut : Datum → TxOut → (w : ℕ) → (v : Value) → TxOut
...
makeWithdrawTxOut : Datum → (scriptIx : ℕ) → TxOut → (w : ℕ) → (v : Value) → List (ℕ × TxOut)
