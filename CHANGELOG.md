...
- Carry the Leios voting key in the pool state: `StakePoolState` pairs the registration parameters with an optional `BlsKeyState` (key and registration epoch); keys expire `maxKeyAgeEpochs` epochs after registration, a bound derived from the KES constants `MaxKESEvoᶜ` and `SlotsPerKESPeriodᶜ` added to `GlobalConstants`, and survive parameter re-registration (`applyFPools`).
