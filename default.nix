...
with nixpkgs; let
  ...
  ledger-src = agdaDeps.ledger;
  agdaWithPkgs = agdaDeps.agda;

  ledger-compiled = stdenv.mkDerivation {
    ...
    installPhase = ''
      ...
      cp simple-ledger-executable-spec.cabal $out

      # Append all the modules generated by MAlonzo to the cabal file
      find $out/MAlonzo -name "*.hs" -print | sed 's#^$out#        #;s#\.hs$##;s#/#.#g' >> $out/simple-ledger-executable-spec.cabal
    '';
  };
in
{
  ledger-hs-package = haskellPackages.callCabal2nix "simple-utxo-ledger-executable-spec" "${ledger-compiled}" {};
