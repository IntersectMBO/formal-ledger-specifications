...
rec {
  ...
  ledgerHsSrc = stdenv.mkDerivation {
    ...
    installPhase = ''
      mkdir -p $out
      agda -c --ghc-dont-call-ghc --compile-dir $out Ledger/Foreign/HSLedger.agda
      cp Ledger/HSLedgerTest.hs $out
      cp Ledger/agda-ledger-executable-spec.cabal $out
  ...
  ledgerHsSrcMidnight = stdenv.mkDerivation {
    pname = "Agda-ledger-midnight-hs-src";
    version = "0.1";
    src = "${agdaLedger}";
    meta = { };
    buildInputs = [ agdaWithPkgs ];
    ...
    installPhase = ''
      mkdir -p $out
      agda -c --ghc-dont-call-ghc --compile-dir $out MidnightExample/HSLedger.agda
      cp MidnightExample/Main.hs $out
      cp MidnightExample/agda-ledger-executable-spec.cabal $out
      # Append all the modules generated by MAlonzo to the cabal file
      find $out/MAlonzo -name "*.hs" -print | sed "s#^$out/#        #;s#\.hs##;s#/#.#g" >> $out/agda-ledger-executable-spec.cabal
    '';
  };
