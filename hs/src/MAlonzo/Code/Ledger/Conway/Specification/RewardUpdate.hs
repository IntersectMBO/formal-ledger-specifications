{-# LANGUAGE BangPatterns #-}
{-# LANGUAGE EmptyCase #-}
{-# LANGUAGE EmptyDataDecls #-}
{-# LANGUAGE ExistentialQuantification #-}
{-# LANGUAGE NoMonomorphismRestriction #-}
{-# LANGUAGE OverloadedStrings #-}
{-# LANGUAGE PatternSynonyms #-}
{-# LANGUAGE RankNTypes #-}
{-# LANGUAGE ScopedTypeVariables #-}

{-# OPTIONS_GHC -Wno-overlapping-patterns #-}

module MAlonzo.Code.Ledger.Conway.Specification.RewardUpdate where

import MAlonzo.RTE (coe, erased, AgdaAny, addInt, subInt, mulInt,
                    quotInt, remInt, geqInt, ltInt, eqInt, add64, sub64, mul64, quot64,
                    rem64, lt64, eq64, word64FromNat, word64ToNat)
import qualified MAlonzo.RTE
import qualified Data.Text
import qualified MAlonzo.Code.Agda.Builtin.Equality
import qualified MAlonzo.Code.Agda.Builtin.Sigma
import qualified MAlonzo.Code.Class.HasAdd.Core
import qualified MAlonzo.Code.Data.Integer.Base
import qualified MAlonzo.Code.Data.Irrelevant
import qualified MAlonzo.Code.Ledger.Conway.Specification.Abstract
import qualified MAlonzo.Code.Ledger.Conway.Specification.Enact
import qualified MAlonzo.Code.Ledger.Conway.Specification.Epoch
import qualified MAlonzo.Code.Ledger.Conway.Specification.Ledger
import qualified MAlonzo.Code.Ledger.Conway.Specification.PParams
import qualified MAlonzo.Code.Ledger.Conway.Specification.Ratify
import qualified MAlonzo.Code.Ledger.Conway.Specification.Rewards
import qualified MAlonzo.Code.Ledger.Conway.Specification.Transaction
import qualified MAlonzo.Code.Ledger.Core.Specification.Epoch

-- _.MaxLovelaceSupplyᶜ
d_MaxLovelaceSupply'7580'_472 ::
  MAlonzo.Code.Ledger.Conway.Specification.Transaction.T_TransactionStructure_24 ->
  Integer
d_MaxLovelaceSupply'7580'_472 v0
  = coe
      MAlonzo.Code.Ledger.Core.Specification.Epoch.d_MaxLovelaceSupply'7580'_338
      (coe
         MAlonzo.Code.Ledger.Conway.Specification.Transaction.d_globalConstants_1416
         (coe v0))
-- _.RandomnessStabilisationWindow
d_RandomnessStabilisationWindow_550 ::
  MAlonzo.Code.Ledger.Conway.Specification.Transaction.T_TransactionStructure_24 ->
  AgdaAny
d_RandomnessStabilisationWindow_550 v0
  = coe
      MAlonzo.Code.Ledger.Core.Specification.Epoch.d_RandomnessStabilisationWindow_94
      (coe
         MAlonzo.Code.Ledger.Conway.Specification.Transaction.d_epochStructure_1878
         (coe v0))
-- _.Slot
d_Slot_630 ::
  MAlonzo.Code.Ledger.Conway.Specification.Transaction.T_TransactionStructure_24 ->
  ()
d_Slot_630 = erased
-- _.SlotsPerEpochᶜ
d_SlotsPerEpoch'7580'_632 ::
  MAlonzo.Code.Ledger.Conway.Specification.Transaction.T_TransactionStructure_24 ->
  Integer
d_SlotsPerEpoch'7580'_632 v0
  = coe
      MAlonzo.Code.Ledger.Core.Specification.Epoch.d_SlotsPerEpoch'7580'_326
      (coe
         MAlonzo.Code.Ledger.Conway.Specification.Transaction.d_globalConstants_1416
         (coe v0))
-- _.addSlot
d_addSlot_728 ::
  MAlonzo.Code.Ledger.Conway.Specification.Transaction.T_TransactionStructure_24 ->
  MAlonzo.Code.Class.HasAdd.Core.T_HasAdd_10
d_addSlot_728 v0
  = coe
      MAlonzo.Code.Ledger.Core.Specification.Epoch.d_addSlot_278
      (coe
         MAlonzo.Code.Ledger.Conway.Specification.Transaction.d_epochStructure_1878
         (coe v0))
-- _.epoch
d_epoch_746 ::
  MAlonzo.Code.Ledger.Conway.Specification.Transaction.T_TransactionStructure_24 ->
  AgdaAny -> AgdaAny
d_epoch_746 v0
  = coe
      MAlonzo.Code.Ledger.Core.Specification.Epoch.d_epoch_90
      (coe
         MAlonzo.Code.Ledger.Conway.Specification.Transaction.d_epochStructure_1878
         (coe v0))
-- _.firstSlot
d_firstSlot_750 ::
  MAlonzo.Code.Ledger.Conway.Specification.Transaction.T_TransactionStructure_24 ->
  AgdaAny -> AgdaAny
d_firstSlot_750 v0
  = coe
      MAlonzo.Code.Ledger.Core.Specification.Epoch.d_firstSlot_92
      (coe
         MAlonzo.Code.Ledger.Conway.Specification.Transaction.d_epochStructure_1878
         (coe v0))
-- Ledger.Conway.Specification.RewardUpdate._._⊢_⇀⦇_,NEWEPOCH⦈_
d__'8866'_'8640''10631'_'44'NEWEPOCH'10632'__2084 a0 a1 a2 a3 a4 a5
  = ()
-- Ledger.Conway.Specification.RewardUpdate._.EpochState
d_EpochState_2088 a0 a1 = ()
-- Ledger.Conway.Specification.RewardUpdate._.NewEpochState
d_NewEpochState_2170 a0 a1 = ()
-- Ledger.Conway.Specification.RewardUpdate._.createRUpd
d_createRUpd_2198 ::
  MAlonzo.Code.Ledger.Conway.Specification.Transaction.T_TransactionStructure_24 ->
  MAlonzo.Code.Ledger.Conway.Specification.Abstract.T_AbstractFunctions_2556 ->
  Integer ->
  MAlonzo.Code.Agda.Builtin.Sigma.T_Σ_14 ->
  MAlonzo.Code.Ledger.Conway.Specification.Epoch.T_EpochState_3332 ->
  Integer ->
  MAlonzo.Code.Ledger.Conway.Specification.Rewards.T_RewardUpdate_3056
d_createRUpd_2198 v0 ~v1 = du_createRUpd_2198 v0
du_createRUpd_2198 ::
  MAlonzo.Code.Ledger.Conway.Specification.Transaction.T_TransactionStructure_24 ->
  Integer ->
  MAlonzo.Code.Agda.Builtin.Sigma.T_Σ_14 ->
  MAlonzo.Code.Ledger.Conway.Specification.Epoch.T_EpochState_3332 ->
  Integer ->
  MAlonzo.Code.Ledger.Conway.Specification.Rewards.T_RewardUpdate_3056
du_createRUpd_2198 v0
  = coe
      MAlonzo.Code.Ledger.Conway.Specification.Epoch.du_createRUpd_3504
      (coe v0)
-- Ledger.Conway.Specification.RewardUpdate._.EpochState.acnt
d_acnt_2226 ::
  MAlonzo.Code.Ledger.Conway.Specification.Epoch.T_EpochState_3332 ->
  MAlonzo.Code.Ledger.Conway.Specification.PParams.T_Acnt_188
d_acnt_2226 v0
  = coe
      MAlonzo.Code.Ledger.Conway.Specification.Epoch.d_acnt_3344 (coe v0)
-- Ledger.Conway.Specification.RewardUpdate._.EpochState.es
d_es_2228 ::
  MAlonzo.Code.Ledger.Conway.Specification.Epoch.T_EpochState_3332 ->
  MAlonzo.Code.Ledger.Conway.Specification.Enact.T_EnactState_1226
d_es_2228 v0
  = coe
      MAlonzo.Code.Ledger.Conway.Specification.Epoch.d_es_3350 (coe v0)
-- Ledger.Conway.Specification.RewardUpdate._.EpochState.fut
d_fut_2230 ::
  MAlonzo.Code.Ledger.Conway.Specification.Epoch.T_EpochState_3332 ->
  MAlonzo.Code.Ledger.Conway.Specification.Ratify.T_RatifyState_1942
d_fut_2230 v0
  = coe
      MAlonzo.Code.Ledger.Conway.Specification.Epoch.d_fut_3352 (coe v0)
-- Ledger.Conway.Specification.RewardUpdate._.EpochState.ls
d_ls_2232 ::
  MAlonzo.Code.Ledger.Conway.Specification.Epoch.T_EpochState_3332 ->
  MAlonzo.Code.Ledger.Conway.Specification.Ledger.T_LState_3010
d_ls_2232 v0
  = coe
      MAlonzo.Code.Ledger.Conway.Specification.Epoch.d_ls_3348 (coe v0)
-- Ledger.Conway.Specification.RewardUpdate._.EpochState.ss
d_ss_2234 ::
  MAlonzo.Code.Ledger.Conway.Specification.Epoch.T_EpochState_3332 ->
  MAlonzo.Code.Ledger.Conway.Specification.Rewards.T_Snapshots_3150
d_ss_2234 v0
  = coe
      MAlonzo.Code.Ledger.Conway.Specification.Epoch.d_ss_3346 (coe v0)
-- Ledger.Conway.Specification.RewardUpdate._.NewEpochState.bcur
d_bcur_2270 ::
  MAlonzo.Code.Ledger.Conway.Specification.Epoch.T_NewEpochState_3404 ->
  MAlonzo.Code.Agda.Builtin.Sigma.T_Σ_14
d_bcur_2270 v0
  = coe
      MAlonzo.Code.Ledger.Conway.Specification.Epoch.d_bcur_3422 (coe v0)
-- Ledger.Conway.Specification.RewardUpdate._.NewEpochState.bprev
d_bprev_2272 ::
  MAlonzo.Code.Ledger.Conway.Specification.Epoch.T_NewEpochState_3404 ->
  MAlonzo.Code.Agda.Builtin.Sigma.T_Σ_14
d_bprev_2272 v0
  = coe
      MAlonzo.Code.Ledger.Conway.Specification.Epoch.d_bprev_3420
      (coe v0)
-- Ledger.Conway.Specification.RewardUpdate._.NewEpochState.epochState
d_epochState_2274 ::
  MAlonzo.Code.Ledger.Conway.Specification.Epoch.T_NewEpochState_3404 ->
  MAlonzo.Code.Ledger.Conway.Specification.Epoch.T_EpochState_3332
d_epochState_2274 v0
  = coe
      MAlonzo.Code.Ledger.Conway.Specification.Epoch.d_epochState_3424
      (coe v0)
-- Ledger.Conway.Specification.RewardUpdate._.NewEpochState.lastEpoch
d_lastEpoch_2276 ::
  MAlonzo.Code.Ledger.Conway.Specification.Epoch.T_NewEpochState_3404 ->
  AgdaAny
d_lastEpoch_2276 v0
  = coe
      MAlonzo.Code.Ledger.Conway.Specification.Epoch.d_lastEpoch_3418
      (coe v0)
-- Ledger.Conway.Specification.RewardUpdate._.NewEpochState.pd
d_pd_2278 ::
  MAlonzo.Code.Ledger.Conway.Specification.Epoch.T_NewEpochState_3404 ->
  MAlonzo.Code.Agda.Builtin.Sigma.T_Σ_14
d_pd_2278 v0
  = coe
      MAlonzo.Code.Ledger.Conway.Specification.Epoch.d_pd_3428 (coe v0)
-- Ledger.Conway.Specification.RewardUpdate._.NewEpochState.ru
d_ru_2280 ::
  MAlonzo.Code.Ledger.Conway.Specification.Epoch.T_NewEpochState_3404 ->
  Maybe
    MAlonzo.Code.Ledger.Conway.Specification.Rewards.T_RewardUpdate_3056
d_ru_2280 v0
  = coe
      MAlonzo.Code.Ledger.Conway.Specification.Epoch.d_ru_3426 (coe v0)
-- Ledger.Conway.Specification.RewardUpdate._.BlocksMade
d_BlocksMade_2338 ::
  MAlonzo.Code.Ledger.Conway.Specification.Transaction.T_TransactionStructure_24 ->
  MAlonzo.Code.Ledger.Conway.Specification.Abstract.T_AbstractFunctions_2556 ->
  ()
d_BlocksMade_2338 = erased
-- Ledger.Conway.Specification.RewardUpdate._.RewardUpdate
d_RewardUpdate_2356 a0 a1 = ()
-- Ledger.Conway.Specification.RewardUpdate._.RewardUpdate.flowConservation
d_flowConservation_2406 ::
  MAlonzo.Code.Ledger.Conway.Specification.Rewards.T_RewardUpdate_3056 ->
  MAlonzo.Code.Agda.Builtin.Equality.T__'8801'__12
d_flowConservation_2406 = erased
-- Ledger.Conway.Specification.RewardUpdate._.RewardUpdate.rs
d_rs_2408 ::
  MAlonzo.Code.Ledger.Conway.Specification.Rewards.T_RewardUpdate_3056 ->
  MAlonzo.Code.Agda.Builtin.Sigma.T_Σ_14
d_rs_2408 v0
  = coe
      MAlonzo.Code.Ledger.Conway.Specification.Rewards.d_rs_3080 (coe v0)
-- Ledger.Conway.Specification.RewardUpdate._.RewardUpdate.Δf
d_Δf_2410 ::
  MAlonzo.Code.Ledger.Conway.Specification.Rewards.T_RewardUpdate_3056 ->
  Integer
d_Δf_2410 v0
  = coe
      MAlonzo.Code.Ledger.Conway.Specification.Rewards.d_Δf_3078 (coe v0)
-- Ledger.Conway.Specification.RewardUpdate._.RewardUpdate.Δf-nonpositive
d_Δf'45'nonpositive_2412 ::
  MAlonzo.Code.Ledger.Conway.Specification.Rewards.T_RewardUpdate_3056 ->
  MAlonzo.Code.Data.Integer.Base.T__'8804'__26
d_Δf'45'nonpositive_2412 v0
  = coe
      MAlonzo.Code.Ledger.Conway.Specification.Rewards.d_Δf'45'nonpositive_3088
      (coe v0)
-- Ledger.Conway.Specification.RewardUpdate._.RewardUpdate.Δr
d_Δr_2414 ::
  MAlonzo.Code.Ledger.Conway.Specification.Rewards.T_RewardUpdate_3056 ->
  Integer
d_Δr_2414 v0
  = coe
      MAlonzo.Code.Ledger.Conway.Specification.Rewards.d_Δr_3076 (coe v0)
-- Ledger.Conway.Specification.RewardUpdate._.RewardUpdate.Δt
d_Δt_2416 ::
  MAlonzo.Code.Ledger.Conway.Specification.Rewards.T_RewardUpdate_3056 ->
  Integer
d_Δt_2416 v0
  = coe
      MAlonzo.Code.Ledger.Conway.Specification.Rewards.d_Δt_3074 (coe v0)
-- Ledger.Conway.Specification.RewardUpdate._.RewardUpdate.Δt-nonnegative
d_Δt'45'nonnegative_2418 ::
  MAlonzo.Code.Ledger.Conway.Specification.Rewards.T_RewardUpdate_3056 ->
  MAlonzo.Code.Data.Integer.Base.T__'8804'__26
d_Δt'45'nonnegative_2418 v0
  = coe
      MAlonzo.Code.Ledger.Conway.Specification.Rewards.d_Δt'45'nonnegative_3086
      (coe v0)
-- Ledger.Conway.Specification.RewardUpdate.RUpdEnv
d_RUpdEnv_2438 ::
  MAlonzo.Code.Ledger.Conway.Specification.Transaction.T_TransactionStructure_24 ->
  MAlonzo.Code.Ledger.Conway.Specification.Abstract.T_AbstractFunctions_2556 ->
  ()
d_RUpdEnv_2438 = erased
-- Ledger.Conway.Specification.RewardUpdate._⊢_⇀⦇_,RUPD⦈_
d__'8866'_'8640''10631'_'44'RUPD'10632'__2440 a0 a1 a2 a3 a4 a5
  = ()
data T__'8866'_'8640''10631'_'44'RUPD'10632'__2440
  = C_RUPD'45'Create'45'Reward'45'Update_2450 AgdaAny |
    C_RUPD'45'Reward'45'Update'45'Exists_2460 |
    C_RUPD'45'Reward'45'Too'45'Early_2468
-- Ledger.Conway.Specification.RewardUpdate._⊢_⇀⦇_,TICK⦈_
d__'8866'_'8640''10631'_'44'TICK'10632'__2470 a0 a1 a2 a3 a4 a5
  = ()
data T__'8866'_'8640''10631'_'44'TICK'10632'__2470
  = C_TICK_2480 MAlonzo.Code.Ledger.Conway.Specification.Epoch.T_NewEpochState_3404
                MAlonzo.Code.Agda.Builtin.Sigma.T_Σ_14
