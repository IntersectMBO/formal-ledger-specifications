...
#        are copied into `_build/website/mkdocs/src/docs/` *and* into
#        `_build/website/mdbook/src/`. The `_build/website/mkdocs/src/mkdocs.yml`
#        and `_build/website/mdbook/book.toml` files are updated:
#
...
#            the `.md` filenames in `_build/website/mkdocs/src/docs/` into a
...
def setup_directories() -> None:
    ...
    if MKDOCS_BUILD_DIR.exists(): # MKDOCS_BUILD_DIR is _build/website/mkdocs/
    ...
    # Avoid deleting unrelated artifacts in _build/ (e.g., from shake/CI)!
    if MDBOOK_BUILD_DIR.exists(): # MDBOOK_BUILD_DIR is _build/website/mdbook/
...
def cleanup_intermediate_artifacts() -> None:
    ...
    (like _build/agda_snapshot_src/ and _build/website/mkdocs/ and _build/website/mdbook/).
    """
...
def copy_staging_to_site_docs(
    agda_docs_staging_dir: Path, # e.g., _build/agda-docs/
    target_site_docs_dir: Path,  # e.g., _build/website/mkdocs/src/docs/ or _build/website/mdbook/src/
...
def generate_mdbook_config(
    mdbook_toml_build_path: Path,      # Target: _build/website/mdbook/book.toml
    mdbook_summary_build_path: Path,   # Target: _build/website/mdbook/src/SUMMARY.md
    book_toml_template_source: Path,   # Source: build-tools/static/mdbook/book.toml
    summary_md_template_source: Path,  # Source: build-tools/static/mdbook/src/SUMMARY.md
    actual_content_files_in_build_src: List[str] # Basenames of .md files in _build/website/mdbook/src/
):
    """
    ...
    if book_toml_template_source.exists():
        try:
            mdbook_toml_build_path.parent.mkdir(parents=True, exist_ok=True) # ensure _build/website/mdbook/ exists
    ...
    # into _build/website/mdbook/src/css/. For example:
    ...
    mdbook_summary_build_path.parent.mkdir(parents=True, exist_ok=True) # ensure _build/website/mdbook/src/ exists

    if summary_md_template_source.exists():
        # Strategy 1: if static SUMMARY.md template exists in build-tools/static/mdbook/src/SUMMARY.md, use it!
...
def main(run_agda_html_flag=False):
    ...
    if MDBOOK_STATIC_DIR.exists(): # MDBOOK_STATIC_DIR is build-tools/static/mdbook/
        try:
            ...
            # This will copy:
            # build-tools/static/mdbook/book.toml -> _build/website/mdbook/book.toml
            # build-tools/static/mdbook/src/* (index.md, SUMMARY.md, css/, js/) -> _build/website/mdbook/src/*
        except Exception as e:
        ...
        MDBOOK_DOCS_DIR.mkdir(parents=True, exist_ok=True) # _build/website/mdbook/src/
        # MDBOOK_CSS_DIR and MDBOOK_JS_DIR would be _build/website/mdbook/src/css and _build/website/mdbook/src/js
    ...
    nav_files_in_docs: List[str] = copy_staging_to_site_docs(
        AGDA_DOCS_STAGING_DIR,
        MKDOCS_DOCS_DIR, # _build/website/mkdocs/src/docs/
        "MkDocs"
    )
    ...
    mdbook_final_content_files: List[str] = copy_staging_to_site_docs(
        AGDA_DOCS_STAGING_DIR,
        MDBOOK_DOCS_DIR, # _build/website/mdbook/src/
        "mdbook"
    )

    # Deploy static assets specifically for mdbook (if any beyond what's in MDBOOK_STATIC_DOCS_DIR)
    # For example, copy custom CSS/JS from MDBOOK_STATIC_CSS_DIR to MDBOOK_CSS_DIR (_build/website/mdbook/src/css)
    ...
    generate_mdbook_config(
        MDBOOK_BUILD_DIR / "book.toml",  # Target: _build/website/mdbook/book.toml
        mdbook_summary_in_build_path,    # Target: _build/website/mdbook/src/SUMMARY.md
        MDBOOK_BOOK_TOML_TEMPLATE,       # Source: build-tools/static/mdbook/book.toml
        MDBOOK_SUMMARY_MD_TEMPLATE,      # Source: build-tools/static/mdbook/src/SUMMARY.md (using your var name)
        mdbook_final_content_files       # List of .md basenames in _build/website/mdbook/src/
    )
