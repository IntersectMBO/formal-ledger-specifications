...
#     bookkeeping (the dashboard) matches the code.
#
# Because status is computed, the anti-drift gate is the dashboard itself: if a
# property's Agda changes (a proof lands, a stub appears) but the committed
# dashboard is not regenerated, `--check` fails. That makes the #413/#414 class
# of drift (a proof exists but the dashboard still says otherwise) a CI failure.
#
# Usage:
#   python3 build-tools/scripts/property-tracking/scan_properties.py          # regenerate dashboard
#   python3 build-tools/scripts/property-tracking/scan_properties.py --check  # CI: fail on drift/staleness
#
# Exit codes: 0 = OK, 1 = catalog error or stale dashboard (see printed report).
#
...
DEFAULT_DASHBOARD = (REPO_ROOT / "build-tools" / "static" / "mkdocs" / "docs"
                     / "ledger-properties-dashboard.md")

BEGIN = "<!-- BEGIN GENERATED: dashboard (scan_properties.py) -->"
END = "<!-- END GENERATED: dashboard (scan_properties.py) -->"
...
def display_path(path: Path) -> str:
    """Repo-relative path for messages, absolute if it lies outside the repo.

    `--dashboard` is caller-supplied and need not be under
    REPO_ROOT (e.g. when dry-running the renderer to a scratch directory).
    """
    try:
...
def validate_catalog(cat: dict):
    ...
    problems = []
    if (not isinstance(cat, dict) or not isinstance(cat.get("meta"), dict)
            or not isinstance(cat.get("properties"), list)):
        return [("ERROR", "<catalog>",
                 "catalog must be a mapping with `meta:` and a `properties:` list")]
    meta = cat.get("meta", {})
    eras = set(meta.get("eras", []))
    sts_vals = set(meta.get("sts_values", []))
    seen = set()
    for i, p in enumerate(cat.get("properties", [])):
        if not isinstance(p, dict):
            problems.append(("ERROR", f"<properties[{i}]>", "entry must be a mapping"))
            continue
        pid = p.get("id")
        if not pid or not isinstance(pid, str):
            problems.append(("ERROR", f"<properties[{i}]>", "property has no (string) id"))
            continue
        if pid in seen:
            problems.append(("ERROR", pid, "duplicate id"))
        seen.add(pid)
        if not p.get("title") or not isinstance(p.get("title"), str):
            problems.append(("ERROR", pid, "missing or non-string `title`"))
        if not isinstance(p.get("module") or "", str):
            problems.append(("ERROR", pid, "`module` must be a string (\"\" if none yet)"))
        defs = p.get("defs") or []
        if not isinstance(defs, list) or not all(isinstance(d, str) for d in defs):
            problems.append(("ERROR", pid, "`defs` must be a list of strings"))
        if not isinstance(p.get("issues") or [], list):
            problems.append(("ERROR", pid, "`issues` must be a list"))
...
def render_region(cat: dict, results: dict) -> str:
    ...
    lines.append(BEGIN)
    lines.append("")
    lines.append("_Generated by `build-tools/scripts/property-tracking/scan_properties.py` from the catalog "
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Derive property status from the Agda and render the dashboard.")
    ap.add_argument("--catalog", type=Path, default=DEFAULT_CATALOG)
    ap.add_argument("--dashboard", type=Path, default=DEFAULT_DASHBOARD,
                    help="generated dashboard (lives in the mkdocs site tree).")
    ap.add_argument("--check", action="store_true",
                    help="CI mode: do not write; fail on a catalog error or a stale dashboard.")
    args = ap.parse_args()
    ...
    results = {}
    if not errors:  # evaluate only a catalog whose shape validated clean
    ...
    rendered = build_file(cat, results,
                          args.dashboard.read_text(encoding="utf-8") if args.dashboard.exists() else None)
