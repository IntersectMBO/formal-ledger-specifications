...
import Agda.Syntax.Common (FileType (..), Induction (..), IsMain (..))
...
defaultPageGen opts srcFile@(FlsInputSourceFile moduleName ft _ _) = do
  logHtml $ render $ "Generating" <+> pretty moduleName <+> "from" <+> (pretty target <> ".")
...
code fileType =
 ...
 where
  ...
  goInline ::
    ...
    -- \^ Nesting depth
    Int ->
    -- \^ Within parenthesis? (disable comma as a VV delimiter)
    [TokenInfo] ->
    [Html]
