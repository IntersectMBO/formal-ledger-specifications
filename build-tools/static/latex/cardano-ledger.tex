...
To say that a predicate \AgdaBound{P} is an \textit{invariant} of a transition rule
means the following: if the transition rule relates states \AgdaBound{s} and \AgdaBound{s'} and if
\AgdaBound{P} holds at state~\AgdaBound{s}, then \AgdaBound{P} holds at state \AgdaBound{s'}.
...
Given a ledger state \AgdaBound{s}, we focus on deposits in the
\UTxOState{} of \AgdaBound{s} that are \GovActionDeposit{}s and we compare that set of
deposits with the \GovActionDeposit{}s of the \GovState{} of \AgdaBound{s}.
When these two sets are the same, we write \AgdaFunction{govDepsMatch}~\AgdaBound{s} and say
the \AgdaFunction{govDepsMatch} relation holds for \AgdaBound{s}.
...
means the following:  if \govDepsMatch{}~\AgdaBound{s} and
\AgdaBound{s}~\AgdaDatatype{⇀⦇}~\AgdaBound{tx}~\AgdaDatatype{,LEDGER⦈}~\AgdaBound{s'}, then
\govDepsMatch{}~\AgdaBound{s'}.
