...
{- module ScriptVerification.LedgerImplementation (Data' : Set)
                                               (serialise : ∀ {A : Set} → A → Data')
                                               (deserialise : ∀ {A : Set} → Data' → Maybe A)
                                               {{_ : DecEq Data'}}
                                               {{_ : DecEq (Data' → Bool)}} where -}

open import ScriptVerification.Prelude

module ScriptVerification.LedgerImplementation
  (scriptImp : _) (open ScriptImplementation scriptImp)
  where
