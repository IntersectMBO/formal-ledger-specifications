...
record Theory {ℓ} : Type (sucˡ ℓ) where
  ...
  ∅-strongly-finite : strongly-finite {A} ∅ˢ
  ∅-strongly-finite = [] , [] , R.SK-sym ∈-fromList

  card-∅ : card (∅ˢ {A} , ∅-strongly-finite) ≡ 0
  card-∅ = refl
  ...
  partialToSet f a = maybe (fromList ∘ [_]) ∅ˢ (f a)

  ∈-partialToSet : ∀ {a : A} {b : B} {f} → f a ≡ just b ⇔ b ∈ partialToSet f a
  ∈-partialToSet {a = a} {b} {f} = mk⇔
    (λ h → subst (λ x → b ∈ maybe (fromList ∘ [_]) ∅ˢ x) (sym h) (to ∈-singleton refl))
    (case f a returning (λ y → b ∈ maybe (λ x → fromList [ x ]) ∅ˢ y → y ≡ just b) of
