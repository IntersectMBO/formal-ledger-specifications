...
```agda
    ...
    ... | yes isV =
      ...
      in do
        ...
        (govSt' , govStep) ← computeGov govΓ govSt (GovProposals+Votes stx)
          ...
          do
            ...
            let certΓ : CertEnv
                certΓ = ⟦ epoch slot , pparams , ListOfGovVotesOf txTop , WithdrawalsOf txTop
                        , allColdCreds govSt enactState ⟧
            (certSt₂ , certStep) ← computeCerts certΓ certState (DCertsOf txTop)
            let govΓ : GovEnv
                govΓ = ⟦ TxIdOf txTop , epoch slot , pparams , ppolicy , enactState
                      , certSt₂ , dom (RewardsOf certSt₂) ⟧
            (govSt₂ , govStep) ← computeGov govΓ govSt (GovProposals+Votes txTop)
            let depositsChange : DepositsChange
                depositsChange = calculateDepositsChange certState₀ certState certSt₂
                  where certState₀ = CertStateOf s
            let utxoΓ : UTxOEnv
                utxoΓ = ⟦ slot , pparams , treasury , utxo₀ , depositsChange , allScripts , allData ⟧
            -- NOTE: assumes LEDGER's UTXOW premise uses (utxoΓ , false)
            (utxoSt₂ , utxoStep) ← computeUtxow (utxoΓ , false) utxoSt txTop
