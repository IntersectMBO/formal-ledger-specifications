...
CERT-poolSet (CERT-pool (POOL-retirepool _))   = ≡ᵉ.refl
CERT-poolSet (CERT-pool (POOL-rereg {pools = pools} {kh = kh} (regd , _)))
  with kh ∈? dom pools
... | yes _ = ≡ᵉ.refl
... | no ¬r = ⊥-elim (¬r regd)
CERT-poolSet (CERT-pool (POOL-reg {pools = pools} {kh = kh} (¬regd , _)))
  with kh ∈? dom pools
... | yes r = ⊥-elim (¬regd r)
...
CERT-rewards-pov (CERT-gov (GOVCERT-regdrep _))   = refl
CERT-rewards-pov (CERT-gov (GOVCERT-deregdrep _)) = refl
CERT-rewards-pov (CERT-gov (GOVCERT-ccreghot _))  = refl
```
...
CERT-deposits-registered reg (CERT-pool (POOL-retirepool _))  = reg
CERT-deposits-registered reg (CERT-pool (POOL-rereg _))       = reg
CERT-deposits-registered reg (CERT-pool (POOL-reg {pools = pools} {kh = kh} {deposits = deps} (¬regd , _))) =
...
CERT-deposits-pov reg (CERT-pool {pp = pp} {stᵈ = stᵈ} {stᵍ = stᵍ}
                       (POOL-reg {pools = pools} {kh = kh} {deposits = deps} (¬regd , _)))
  with kh ∈? dom pools
...
CERT-deposits-pov _ (CERT-pool (POOL-rereg {pools = pools} {kh = kh} (regd , _)))
  with kh ∈? dom pools
... | yes _ = refl
... | no ¬r = ⊥-elim (¬r regd)

CERT-deposits-pov _ (CERT-pool (POOL-retirepool _)) = refl

CERT-deposits-pov _ (CERT-gov
                     (GOVCERT-regdrep {d = d} {c = c} {stᵈ = stᵈ} {stᵖ = stᵖ} {depositsᵍ = deps} _)) =
  begin
...
CERT-deposits-pov _ (CERT-gov
                     (GOVCERT-deregdrep {c = c} {d = d} {depositsᵍ = deps} {depositsᵈ = depsᵈ} {stᵖ = stᵖ} (_ , cd∈))) =
  begin
