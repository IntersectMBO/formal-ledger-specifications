...
\begin{itemize}
  ...
  \item \AgdaFunction{÷₀}: Division operator that returns zero when the denominator is zero.
    This result is sensible when a pool has no stake.
\end{itemize}
...
\begin{code}[hide]
_÷₀_ : ℚ → ℚ → ℚ
_÷₀_ x y = case y ≟ 0 of λ where
  (yes _) → 0
  (no y≢0) → let instance nonZero-y = ℚ.≢-nonZero y≢0 in x ÷ y
\end{code}
\begin{code}
...
rewardOwners rewards pool ownerStake stake = case rewards ≤? cost of
\end{code}
\begin{code}[hide]
    λ where
\end{code}
...
rewardMember rewards pool memberStake stake = case rewards ≤? cost of
\end{code}
\begin{code}[hide]
    λ where
\end{code}
...
\begin{itemize}
  \item \AgdaField{Δt}: The change to the treasury. This will be a positive value.
  \item \AgdaField{Δr}: The change to the reserves. This will be a negative value.
  \item \AgdaField{Δf}: The change to the fee pot. This will be a negative value.
...
The ``Reserves'' will be depleted eventually.
