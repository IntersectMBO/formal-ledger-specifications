...
instance
  ...
  GOVToConf .convⁱ deposits (L.GOV-Vote (a , b , c , d)) = C.GOV-Vote (a , b , deposits ⊢conv c , d)
  GOVToConf .convⁱ deposits (L.GOV-Propose h) = C.GOV-Propose h

  GOVSToConf : ∀ {Γ s votes s' n}
             → L.Deposits × L.Deposits
  ...
  GOVSToConf .convⁱ deposits (BS-base Id-nop) = BS-base Id-nop
  GOVSToConf .convⁱ deposits (BS-ind r rs)     = BS-ind (deposits ⊢conv r) (deposits ⊢conv rs)

  GOVFromConf : ∀ {Γ s votes s' n}
            → (Γ , n) C.⊢ s ⇀⦇ votes ,GOV⦈ s' ⭆ (conv Γ , n) L.⊢ s ⇀⦇ votes ,GOV⦈ s'
  GOVFromConf .convⁱ _ (C.GOV-Vote (a , b , c , d)) = L.GOV-Vote (a , b , conv c , d)
  GOVFromConf .convⁱ _ (C.GOV-Propose h)        = L.GOV-Propose h

  GOVSFromConf : ∀ {Γ s votes s' n}
  ...
  GOVSFromConf .convⁱ _ (BS-base Id-nop) = BS-base Id-nop
  GOVSFromConf .convⁱ _ (BS-ind r rs)    = BS-ind (conv r) (conv rs)
