...
* If the current reward update is empty and `s`{.AgdaBound} is greater than the
  ...
  `StabilityWindow`{.AgdaBound}, then a new rewards update is
...
* If the current reward update is empty and `s`{.AgdaBound} is less than or
  ...
  StabilityWindow, then the state is not updated.
...
data _⊢_⇀⦇_,RUPD⦈_
  ...
  CreateRewardUpdate : ∀ {b es s} {ru' : RewardUpdate} →
    let
      ru' = createRUpd SlotsPerEpochᶜ b es {! MaxLovelaceSupply !}
    in
    ∙ s > firstSlot (epoch s) + StabilityWindow
    ───────────────────────────────────────────
