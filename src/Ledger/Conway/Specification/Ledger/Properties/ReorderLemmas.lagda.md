...
source_path: src/Ledger/Conway/Specification/Ledger/Properties/ReorderLemmas.lagda.md
---

# Reordering lemmas (proof layer) {#sec:ledgers-reorder-lemmas}
...
theorem; the narrative documentation of everything in this file lives there.

<!--
```agda
{-# OPTIONS --with-K #-}
-- NOTE: this module makes NO postulates.  Everything the reordering
-- development assumes is a *parameter* of the `Assuming` module below;
-- `Reorder` declares the corresponding postulates and instantiates
-- `Assuming` with them.
...
module Ledger.Conway.Specification.Ledger.Properties.ReorderLemmas
...
## Vocabulary (shared with the postulate statements in `Reorder`)
...
## The proofs, parameterized by the postulated facts

```agda
module Assuming
  (replay-outs-disjoint : ∀ {Γ s s′ l} → Γ ⊢ s ⇀⦇ l ,LEDGERS⦈ s′
  ...
  (Ins#Outs-exec : ∀ {Γ s s′ l} → Γ ⊢ s ⇀⦇ l ,LEDGERS⦈ s′ → Ins#Outs l)
  where
