...
module Ledger.TokenAlgebra.ValueSetList (PolicyId AssetName : Type) where
...
fromListMap : ∀ {A B : Type} → List (A × B) → A ⇒ B
fromListMap = {!!}
...
\begin{code}
  ...
  Value-TokenAlgebra :
    ...
    --------------------------------------
    → TokenAlgebra
  Value-TokenAlgebra specialPolicy specialAsset = record
    { Value                     = {!!} -- List (AssetId × Quantity)
    ; coin                      = λ x → {!totalMap↠coin (fromListTotalMap x)!} -- totalMap↠coin
    ; inject                    = λ x → {!!} -- coin↪totalMap
    ; policies                  = {!!} -- policies
    ; size                      = {!!} -- size
    ; _≤ᵗ_                      = {!!} -- leq
    ; AssetName                 = {!!} -- AssetName
    ...
    ; coinIsMonoidHomomorphism  = CoinMonHom -- CoinMonHom
