...
module _ {a} {A : Set a} where
  ...
  maxlen≤∷ : (ls : List (List A))(l : List A) → maxlen ls ≤ maxlen (l ∷ ls)
  maxlen≤∷ [] _ = z≤n
  maxlen≤∷ (l' ∷ ls) l = subst (maxlen (l' ∷ ls) ≤_) refl (x≤y⊔x (maxlen (l' ∷ ls)) (length l))

  ∈-maxlen-≤ : {ls : List (List A)} → ∀ l → l ∈ ls → length l ≤ maxlen ls
  ∈-maxlen-≤ {l ∷ ls} .l (here refl) = subst (length l ≤_) refl (x≤x⊔y (length l) (maxlen ls))
...
module _ {a} {A : Set a} where
  ...
  ∈→∈-insert {_ ∷ _} x∈ _ (here refl) = there x∈
  ∈→∈-insert {_ ∷ _} (here refl) l (there l∈) = ∈-map l l∈
  ∈→∈-insert {_ ∷ _} {y} {x} (there x∈) l (there l∈) with map∷decomp l∈
