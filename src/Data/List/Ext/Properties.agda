...
map-∷ʳ : {a b : Level} {A : Set a} {B : Set b} (f : A → B) (l : List A) {a : A}
         → map f l ∷ʳ f a ≡ map f (l ∷ʳ a)
map-∷ʳ f [] = refl
map-∷ʳ f (x ∷ l) = cong (f x ∷_) (map-∷ʳ f l)

-- TODO: try to find standard library equivalents for these
module _ {a b : Level} {A : Set a} {B : Set b} where
  frominj₁ : List (A ⊎ B) → List A
  frominj₁ [] = []
  frominj₁ (inj₁ v ∷ vps) = v ∷ frominj₁ vps
  frominj₁ (inj₂ _ ∷ vps) = frominj₁ vps

  frominj₂ : List (A ⊎ B) → List B
  frominj₂ [] = []
  frominj₂ (inj₁ _ ∷ vps) = frominj₂ vps
  frominj₂ (inj₂ p ∷ vps) = p ∷ frominj₂ vps

  ∈-frominj₁ : {ll : List A} {lr : List B} → frominj₁ (map inj₁ ll ++ map inj₂ lr) ≡ ll
  ∈-frominj₁ {[]} {[]} = refl
  ∈-frominj₁ {[]} {y ∷ lr} = ∈-frominj₁{lr = lr}
  ∈-frominj₁ {x ∷ ll} {lr} = cong (x ∷_) ∈-frominj₁

  ∈-frominj₂ : {ll : List A} {lr : List B} → frominj₂ (map inj₁ ll ++ map inj₂ lr) ≡ lr
  ∈-frominj₂ {[]} {[]} = refl
  ∈-frominj₂ {[]} {y ∷ lr} = cong (y ∷_) (∈-frominj₂ {[]})
  ∈-frominj₂ {x ∷ ll} {lr} = ∈-frominj₂{ll}
  ...
  dec-is-inj₁ = λ {(inj₁ _) → yes tt; (inj₂ _) → no λ ()}
