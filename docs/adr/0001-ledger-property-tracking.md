<!-- File: `docs/adr/0001-ledger-property-tracking.md` -->
...
There are three important files to know about:

+  `properties.yaml`: the properties catalog that we curate by hand.
+  `ledger-properties-dashboard.md`: the generated list or "dashboard" of properties and their status.
+  `ledger-properties-issues.md`: the generated list of GitHub issues pertaining to properties.


## Context

We are formalizing and proving properties of the ledger (preservation of value,
governance invariants, the items under issue #45, …) in two eras simultaneously:
**Conway** and **Dijkstra**.  The work has no shared organization, and three
representations of each property drift apart.
...
These were discovered upon auditing for this ADR.
...
So two readers can disagree about whether #413 or #414 is "the done one."
The tracking system will help us avoid such disagreements.
...
| What properties exist, their identity, location, tracking issue | **The catalog** (version-controlled; declares no status)          | `build-tools/properties.yaml` |
...
## Rationale for keeping the issues and property dashboard in the repo

We have a [GitHub Project board](https://github.com/orgs/IntersectMBO/projects/79)
and we discussed using that alone for property-issue tracking.  However, a project
board is invisible to the code and to automation; a fresh clone, CI, and a fresh
Claude Code session can not see it; but all can see the catalog and dashboard that are
maintained in the repo.

### Status is derived, not asserted

The `properties.yaml` catalog declares **no** status field.

`build-tools/scripts/scan_properties.py` uses a property's `module` reference and the Agda
on disk to classify each property by its status, which is one of four classes.
...
Because status is computed, the anti-drift gate is the generated dashboard itself:
...
dashboard is not regenerated and committed, CI fails. The Agda `--safe` typecheck in
...
`proved`); run `scan_properties.py` and commit the regenerated dashboard; close (or
...
the Agda or the dashboard is stale. `gh_project_render.py` pulls issue open/closed
state into a separate coordination view
(`build-tools/static/mkdocs/docs/ledger-properties-issues.md`),
so the two generators never write the same file; `gh_project_populate.py` seeds
issues/labels from the catalog. (Both use `gh` and are run locally — see
`build-tools/scripts/README.md`.)

## Reconciliation actions (initial)

Snapshot at the time of writing — see the live dashboard for current state.
...
The repo-local catalog and dashboard are the source of truth; the following surface it
without becoming a second place the status can live:

- **mkdocs site**. The dashboard is generated directly into the site tree
  (`build-tools/static/mkdocs/docs/ledger-properties-dashboard.md`) and published as the
  **Properties Dashboard** page (top-level nav). It is generated, not hand-edited, and
  `scan_properties.py --check` fails if it is stale — so the site can never show a
  stale dashboard. The generated issues view lives in the same directory.
...
- Small ongoing cost: edit the catalog and regenerate the dashboard when a
