...
open import Class.Foldable using (fold)
open import Class.Functor using (_<$>_) renaming (fmap to map)
open import Class.Bifunctor using (map₁; map₂)
open import Class.Decidable using (_⁇; ifᵈ_then_else_; _⁇²; dec)
∃⁇ : ∀ {ℓ} → Set (sucˡ ℓ)
∃⁇ {ℓ} = Σ (Set ℓ) _⁇
open import Class.Monad using (return; _>>=_; _>>_; _=<<_; mapM; filterM)
open import Class.Semigroup using (Semigroup; _◇_; _<>_)
open import Class.DecEq using (DecEq-×); instance _ = DecEq-×
...
private
  ...
  -- * The tactic works for nested tuples.
  data ℚ× {A : Set} ⦃ _ : Semigroup A ⦄ ⦃ _ : Monoid A ⦄ : A → A → Set where
    base :
      ────────────────────────────────
      ℚ× ε ε
  ...
  module _ ⦃ _ : ∀ {A} ⦃ _ : Semigroup A ⦄ ⦃ _ : Monoid A ⦄ → ℚ× {A} ⁇² ⦄ where
    ...
    module _ {A} ⦃ _ : Semigroup A ⦄ ⦃ _ : Monoid A ⦄
             (n m n′ m′ : A) (𝕢ˡ : ℚ× n m) (𝕢ˡ˘ : ℚ× m n) (𝕢ʳ : ℚ× n′ m′) where
      _ : ℚ×-step-premises .proj₁
      _ = (𝕢ˡ , 𝕢ˡ˘) , 𝕢ʳ
